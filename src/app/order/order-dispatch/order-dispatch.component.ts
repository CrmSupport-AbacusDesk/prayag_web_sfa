...
export class OrderDispatchComponent implements OnInit {
    ...
    update_order()
    {
        ...
        this.serve.fetchData({'order_id':this.order_id,"data":this.order_item,'dr_id':this.order_detail.dr_id,'uid':this.uid ,'loginid':this.userId,'uname':this.userName},"Order/dispatch_order")
        .subscribe(resp=>{
            console.log(resp);

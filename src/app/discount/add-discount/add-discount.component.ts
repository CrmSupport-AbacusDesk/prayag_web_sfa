...
total:any=0;
   getDiscountData(id)
   {
      this.serve.fetchData({'id':id},"Discount/discount_detail").subscribe((result=>
      {
          console.log(result);
  ...
  addDiscount()
  {
    ...
    this.serve.fetchData({"detail":this.detail,"id":this.detail['id'],'uid':this.userId,'uname':this.userName},"Discount/update_discount_detail").subscribe((result=>{
      console.log(result)
      if(result){
        this.rout.navigate(['/discount-list'])
      }
